...
class AnalysisAgentResponseSchema(AgentResponseSchema):
    plot_file_dict: Dict[str, str] = Field(str, description="Dictionary of names of the plot files generated by the analysis as keys and description of plot as values.")
...
def summarize_analysis(state: PlanExecuteState, analysis_response: str, plot_file_dict:Dict=None) :
...
def analysis_agent_node(state: PlanExecuteState) -> Command[Literal["replanner"]]:
    ...
    summary = summarize_analysis(state, response_structured.summary, response_structured.plot_file_dict)
    ...
    return Command(goto="replanner", update={"messages": [
            ...
            ], 'past_steps': [(state["next_agent"], response_structured.summary, response_structured.status)], 'markdown_report': summary})
